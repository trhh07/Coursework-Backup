...
def auto(ammoC):                 #sub program to set up automatic firing
...
def semi(ammoC):             #sub program to set up semi automatic firing
    #semi firing code
    arduino.write(b'2')
    time.sleep(0.25)
    arduino.write(b'3')
...
def burst(ammoC):        #sub program to set up burst firing
    #burst firing code
    for i in range(2):
        arduino.write(b'2')
        time.sleep(0.25)
    ...
    while state == 'ON':
        ...
        while True:    #starts a while loop to repeat lines 51-124 indefintely until a file change is detected
            ...
            for contour in cnts:
                ...
                if x > 700 and x < 1150:       #checks to see if movement is within the line of fire of the gun with extra leeway one either side to account for timings for firing
                    print('fire')
                    arduino.write(b'1')
                    ...
                    txt = change(txt, 'tally.txt', False)    #updates tally file to indicate when the system detects movement
        ...
        while state == "OFF": # keeps program idle till its turned on
            with open('state.txt') as onOff:
                state = onOff.read()
